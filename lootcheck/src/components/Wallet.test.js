...
describe('Wallet', () => {
    ...
    it('creates an input to deposit or withdraw from balance ', () => {
       expect(
           wallet.find('.input-wallet').exists()
       ).toBe(true);
    });
    ...
    describe('when the user types into the wallet input', () => {
        ...
        it('updates the local wallet balance in state and converts it to a number', () => {
            ...
            ).toEqual(parseInt(userBalance, 10));
        });

        describe('ans the user wants to make a deposit', () => {
            beforeEach(() => {
            ...
            it('dispatches the "deposit()" it receives from props with local balance', () => {
                expect(
                    mockDeposit
                ).toHaveBeenCalledWith(parseInt(userBalance, 10));
            });
        });
        ...
        describe('and the user wants to make a withdrawal', () => {
            ...
            it('dispatches the "withdraw() which it receives from props with the loacl withdrawal"', () => {
                expect(
                    mockWithdraw
                ).toHaveBeenCalledWith(parseInt(userBalance, 10));
            });
